...
describe('PhotoGallery', () => {
  ...
  it('displays empty state when no photos and no error', () => {
    ...
    expect(screen.getByText(/No photos are available for the selected filters/)).toBeInTheDocument();
  });
  ...
  it('opens image modal on photo click', () => {
    ...
    const photoCard = screen.getByText('🔴 Curiosity').closest('.cursor-pointer');
  ...
  it('calls onLoadMore when provided', () => {
    ...
    render(<PhotoGallery photos={mockPhotos} hasMore={true} onLoadMore={mockLoadMore} />);

...
jest.mock('./VersionFooter', () => {
  return function MockVersionFooter({ minimal, className }: { minimal?: boolean; className?: string }) {
    return <div data-testid="version-footer" className={className}>Version v1.3.0</div>;
  };
});
...
describe('Navigation', () => {
  ...
  test('navigation links have correct hrefs', () => {
    ...
    expect(screen.getByText('Space Images').closest('a')).toHaveAttribute('href', '/apod');
    expect(screen.getByText('Mars Rovers').closest('a')).toHaveAttribute('href', '/mars-rovers');
    expect(screen.getByText('NEO Tracker').closest('a')).toHaveAttribute('href', '/neo-tracker');
  ...
  test('handles click outside mobile menu', () => {
    ...
    const clickHandler = mockAddEventListener.mock.calls.find(call => call[0] === 'click')?.[1];
    if (clickHandler) {
      const mockEvent = {

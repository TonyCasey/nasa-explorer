...
export class NASAService {
  ...
  static async getAPOD(date?: string): Promise<APODResponse> {
    ...
    const response = await api.get<{ success: boolean; data: APODResponse }>('/apod', { params });
  ...
  static async getAPODRange(
    ...
    const response = await api.get<{ success: boolean; data: APODResponse[] }>('/apod/range', {
  ...
  static async getMarsRoverPhotos(params: {
    ...
    const response = await api.get<{ success: boolean; data: { photos: MarsRoverPhoto[] } }>(
      '/mars-rovers/photos',
      {
        params: {
          rover: params.rover,
  ...
  static async getRoverInfo(roverName: string): Promise<any> {
    ...
    const response = await api.get<{ success: boolean; data: any }>(`/mars-rovers/${roverName}`);
  ...
  static async getNEOById(neoId: string): Promise<NEOObject> {
    const response = await api.get<{ success: boolean; data: NEOObject }>(`/neo/${neoId}`);
    return response.data.data;
  }
  ...
  static async getEPICImages(date?: string): Promise<any[]> {
    const params = date ? { date } : {};
    const response = await api.get<{ success: boolean; data: any[] }>('/epic', { params });
    return response.data.data;
  }

  // Health check
  static async healthCheck(): Promise<{ status: string; timestamp: string }> {
    const response = await api.get<{ success: boolean; data: { status: string; timestamp: string } }>('/health');
    return response.data.data;
  }
}

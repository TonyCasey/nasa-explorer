...
const mockComponents = {
  ...
  FavoriteButton: ({ onClick }: any) => (
    <button data-testid="favorite-btn" onClick={onClick}>
      ❤️
    </button>
  ),
  ImageViewer: ({ src, alt }: any) => (
    <img data-testid="image-viewer" src={src} alt={alt} />
  ),
  ...
  RoverFilters: ({ filters, onChange }: any) => (
    <div data-testid="rover-filters">
      <select
        value={filters.rover}
        onChange={(e) => onChange({ ...filters, rover: e.target.value })}
      >
...
describe('Mega Frontend Coverage Suite', () => {
  ...
  describe('Component Rendering Coverage', () => {
    ...
    it('should render StatusIndicator with different statuses', () => {
      ...
      statuses.forEach((status) => {
        const { unmount } = render(
          <mockComponents.StatusIndicator status={status} />
        );
    ...
    it('should render MetricCard with data', () => {
      ...
      expect(screen.getByTestId('metric-card')).toHaveTextContent(
        'Users: 1,234'
      );
    });
    ...
    it('should render DatePicker with value and change handler', () => {
      ...
      render(
        <mockComponents.DatePicker value="2025-08-15" onChange={mockChange} />
      );
